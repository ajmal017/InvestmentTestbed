...
if 1:
    price_list = db.select_query("select a.cd, b.date, b.oepn, b.close"
                                 "  from index_master a, index_price b"
                                 " where a.cd = b.idx_cd")
    price_list.columns = ['cd', 'date', 'open', 'close']
    ...
    pivoted_close_price_datas = reference_datas.pivot(index='date', columns='cd', values='close').fillna(method='ffill')
    pivoted_open_price_datas = reference_datas.pivot(index='date', columns='cd', values='open').fillna(method='ffill')
