...
actor_saver = "data/5.actor_weights.h5"
critic_saver = "data/5.critic_weights.h5"
...
def BuildActorCritic():
    ...
    xConv = Conv2D(filters = 16, kernel_size=(5, 5), activation='relu')(inputS)
    state = Flatten()(xConv)
...
def Training(actor, policy, critic, data):
    ...
    for i in range(start, len(data) - step, step):
        ...
        if t < T:
            ...
            # 연평균 포트폴리오의 수익률과 분산을 계산해서 Sharp ratio를 구한다.
...
#                nextAction = ChooseAction(policy, nextState)
        ...
        if terminal:
            ...
            currState = nextState
            currAction = ChooseAction(policy, nextState)
